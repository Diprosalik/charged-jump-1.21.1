<div align="center">
  <img src="https://cdn.modrinth.com/data/cached_images/8a1259f7a28a21a40d57d5be0c00715c9b1cf50e_0.webp" alt="Charge Jump Banner" width="600">

  # ⚡ Charged Jump
  ### *Master the rhythm of sprinting to unleash powerful leaps!*

  [![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
  [![Platform: Fabric](https://img.shields.io/badge/Platform-Fabric-lightgrey.svg)](https://fabricmc.net/)
</div>

<br>

---

<br>

## **Features**



<br>

### **Gain Speed / Charge Jump**



Run for more than 1 second to gain extra Speed and you will be followed by some charging particles

Gets canceled after falling more than 2 Blocks

![gain speed](https://cdn.modrinth.com/data/cached_images/23cf5244ced235e15be2c11439cc7654a90852d0.gif)

<br>

### **Charged Jump**



After gaining speed and charging the jump you are able to jump much farther

After a charged jump you have to charge your jump again

#### **Difference on range in normal conditions:**
| Action | Distance |
| :--- | :--- |
| 🚶 Walking | 2 Blocks |
| 🏃 Sprinting | 4 Blocks |
| ⚡ **Charged Jump** | **~6 Blocks** |


<br>

## **Configuration**
Thanks to **Cloth Config**, this mod is **fully customizable**! You can adjust the following settings:
* **Particle Effects:** Toggle the charging particles on or off.
* **Charge Time:** Change how long you need to sprint until the jump is ready.

> **Recommendation:** Install **ModMenu** to access these settings directly in-game via the mod list!

<br>

## **Dependencies**
To ensure the mod works correctly, the following are required:
* **Fabric API**
* **Cloth Config API**

<br>

## **Planned Features**

- More **configurations** for the Mod
- **Enchantement** for the ability to charge jump

<br>

---
<div align="center">
  <sub>Version 1.0.1-alpha.</sub>
  <br>
  ⚠️ Note: This is an Alpha version. Please report any bugs via the Issue Tracker!
</div>
